...
public class Cky {
 ...
 /*
                      System.out.println("Evaluating: " + prods.getLeftSide().get().get(0) + "->" + prods.getRightSide().get().get(0) + prods.getRightSide().get().get(1));
    ...
    private List<Production> findProductions(String first, String second) {
        ...
        return this.grammar.getProductions()
                .stream()
                .filter(production -> production.getRightSide().get().size() > 1 && production.getRightSide().get().get(0).equals(first) && production.getRightSide().get().get(1).equals(second))
                .collect(Collectors.toList());
