...
public class EventHandler implements EventInterface{
    ...
    public EventHandler(GameList<AdvObject> inventory, GameList<AdvObject> objList, GameList<AdvObjectContainer> contList, GameList<Npc> npcList, GameList<Door> doorList, GameList<Lock> lockList,JTextArea out,GameList<Room> roomList,GameList<Dialog> dialogList) {
    ...
    public void check(ParserOutput p, Room currentRoom, boolean move) {
        ...
        if(!flags[0]){
            ...
            if(p.getCommand().getType() == CommandType.USE && p.getObject().getId() == objList.getById(55).getId() && !inventory.contains(objList.getById(61))){//usa il pc ti spedisce una pala
        ...
        } else if(!flags[4]){//finire eventi del torrione
